...
*/
	...
	mymeas.GetTimingCFD(cfd_x, 100, 150, 0, true, 0, false); // this creates the timing_results matrix/vector

	// apply cut for time difference between two channels
	mymeas.SkipEventsTimeDiffCut(10, 11, 0.3736-0.4, 0.3736+0.4, false); //skip all events which time diff is not of interest to us
	mymeas.SkipEventsTimeDiffCut(12, 13, 0.465599-0.4, 0.465599+0.4, false); //skip all events which time diff is not of interest to us
	...
	//vector<float> ly_C0_andrea = {1990.6, 2032.78, 1901.56, 1881.76, 1897.96, 1903.25, 1958.15, 1865.21}; //andrea_C0: selection with 1ns window (time_cut_andrea_C0.png) + large intw (intw_andrea.png)
	//vector<float> ly_C0_andrea = {2027.09, 2066.08, 1959.58, 1949.93, 1944.87, 1950.03, 1981.46, 1922.22}; //andrea_C0: selection with .8ns window (time_cut_andrea_C0.png) + large intw (intw_andrea.png)
	//vector<float> ly_C0_andrea = {1613.69, 1692.17, 1516.59, 1475.38, 1477.6, 1515.11, 1603.31, 1464.49}; //andrea_C0: selection with .5ns window (time_cut_andrea_C0.png) + large intw (intw_andrea.png)
	//vector<float> ly_C0_andrea = {1903.1, 1957.7, 1803.56, 1779.5, 1804.67, 1814.45, 1871.41, 1763.31}; //andrea_C0: selection with 1.2ns window (time_cut_andrea_C0.png) + large intw (intw_andrea.png)
	vector<float> ly_C0_andrea = {1624.09, 1703.68, 1517.3, 1501.99, 1518.07, 1517.47, 1599.56, 1480.77}; //andrea_C0: selection with 0.8ns window (time_cut_andrea_C0_new.png) + large intw (intw_andrea.png)
	...
	mymeas.Print_Phi_ew(phi_chx_andrea, ly_C0_andrea, SiPMchannels, intwindowminus, intwindowplus, findmaxfrom, findmaxto, 200, true, false);
	mymeas.Print_Phi_ew(phi_chx_andrea, ly_C0_andrea, SiPMchannels, intwindowminus, intwindowplus, findmaxfrom, findmaxto, 200, false, false);
